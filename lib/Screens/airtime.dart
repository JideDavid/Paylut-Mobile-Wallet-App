...
class _AirtimeState extends State<Airtime> {
  ...
  saveNewNumber() async{
    ...
    .collection('users').doc(userDetails.uid)
    .collection('savedNumbers').doc(phoneNum).get();
    ...
    ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('$phoneNum has been added to your list'),
  ...
  updateUseTime() async{
    DocumentSnapshot snapshot = await FirebaseFirestore.instance
        .collection('users').doc(userDetails.uid).collection('savedNumbers')
        .doc(phoneNum).get();

    if(snapshot.exists){
      int initialUseTime = snapshot.get('useTime');
      await FirebaseFirestore.instance.collection('users').doc(userDetails.uid)
      .collection('savedNumbers').doc(phoneNum).update({'useTime': initialUseTime + 1});
    }else {
      if(kDebugMode){
  ...
  removeFromDataBase(int index)async{
    await FirebaseFirestore.instance
        ...
        .doc(userDetails.uid)
        .collection('savedNumbers')
        .doc(savedNumbers[index].number).delete();
  }
            ...
            TextField(
              ...
              onChanged: (value) {
                if (value.isEmpty) {
                  ...
                  amount = ((int.tryParse(value) != null) ? int.tryParse(value) : 0)!;
                  if (amount > 0) {
                    hasAmount = true;
                    if (kDebugMode) {
                ...
                child: Container(
                  ...
                  child: Column(
                    children: [
                      ...
                      savedNumbers.isEmpty ? const Expanded(
                        child: Column(
                          mainAxisAlignment: MainAxisAlignment.center,
                          children: [
                            Row(
                                            ...
                                            child: Row(
                                              ...
                                              children: [
                                                Icon(
                                                  Icons.circle,
                                                  size: 12,
                                                  color: savedNumbers[index].providerIndex == 1
                                                      ? Colors.yellow
                                                      : savedNumbers[index].providerIndex == 2
                                                          ? Colors.red
                                                          : savedNumbers[index].providerIndex == 3
                                                              ? Colors.green
                                                              : Colors.green[900],
                                                ),
                                                const SizedBox(
